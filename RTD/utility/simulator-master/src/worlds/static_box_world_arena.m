classdef static_box_world_arena < world
    properties
        buffer = 1 ;
        obstacles_unseen
        obstacles_seen
        obstacle_size_bounds = [4,4] ;
        obstacle_rotation_bounds = [-0.01,0.01] ;
        bounds_as_polyline
    ...
    methods
        %% constructor
        function W = static_box_world_arena(varargin)
            % set default properties
            bounds =  [-430,-390,-640,-610]; 
            N_obstacles = 0 ;
...
%             yhi = yhi - 2*b(2) ;
            ...
            if isempty(W.start)
                W.vdisp('Generating start pose',5)
                s = [-425; -632.2; 0];
                W.start = s ;   
            end
...
%                 ylo = ylo + b ; yhi = yhi - b ;
                ...
                obs_x = [-408.41, -424.97, -393.13];
                obs_y = [-635.66, -615.12, -615.29];
