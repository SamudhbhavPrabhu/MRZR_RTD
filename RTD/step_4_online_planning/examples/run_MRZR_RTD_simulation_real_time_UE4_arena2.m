...
%N_obstacles = 8 ;
bounds = [-428, -357, -640, -595]; % [-460,-330,-660,-530];%[480,585,-145,-100]; %%% %bounds =  [-108, -49.7, 96, 148]; 
goal_radius = 3 ;
...
world_info.goal = [-367.6; -616.4];%[-348.2; -545.4];%[574.2; -139]; % %% % [-60;136];
...
Z = zeros(5,50);
agent_state = [-417.5; -615.9; 0; 0; 0]; %[-441.8; -632.2; 0;0;0];
agent_estim_state = agent_state;
j = 0;

while(myRate.TotalElapsedTime<30)
