classdef static_box_world_arena1 < world
    properties
        buffer = 1 ;
        obstacles_unseen
        obstacles_seen
        obstacle_size_bounds = [4,4];%[0.3,0.7] ;
        obstacle_rotation_bounds = [-0.01,0.01];%[-pi,pi] ;
        bounds_as_polyline
    ...
    methods
        %% constructor
        function W = static_box_world_arena1(varargin)
            % set default properties
            bounds =  [-460,-330,-660,-530] ; %[-20,20,-10,10];
            N_obstacles = 0 ;
...
%                 
%                 xlo = B(1) ; xhi = B(2) ; ylo = B(3) ; yhi = B(4) ;
%                 xlo = xlo + b(1) ; xhi = xhi - b(1) ;
%                 ylo = ylo + b(2) ; yhi = yhi - b(2) ;

                obs_x = [-408.41, -424.97, -393.13, -414.70, -363.30, -372.03, -332.72, -396.42];
                obs_y = [-635.66, -615.12, -615.29, -588.53, -599.65, -569.21, -569.02, -558.30];
